...
const columns: GridColDef[] = [
  ...
  { field: 'itemVariant', headerName: 'Item Variant', width: 200 },
...
const StockTable = ({ warehouse }) => {
  ...
  useEffect(() => {
    try {
      const fetchStockData = async () => {
        const response = await axios.get("http://localhost:8080/api/stock/" + warehouse, {
          headers: {
            //@ts-ignore
  ...
  const doubleClick = (e : any) => {
    setOpen(true)
    setLoading(true)
    axios.get("http://localhost:8080/api/stock/" + warehouse + "/id/" + e.id, {
      headers: {
        //@ts-ignore
        Authorization: `Bearer ${session.accessToken}`, // Use template literals
      }
  ...
  return (
    <>
      ...
      <Dialog maxWidth={"md"} fullWidth={true} onClose={handleClose} open={open}>
        ...
        <DialogActions>
          { !loading ? <Button variant='contained' color={"error"}>Delete</Button> : null}
          { !loading ? <Button variant='contained' color={"success"}>Save</Button> : null}
          { !loading ? <Button variant='contained' color={"primary"}>Close</Button> : null}
        </DialogActions>
      </Dialog>
    </>
  );
